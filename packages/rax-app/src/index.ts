import { IGetBuiltInPlugins, IPluginList, Json, IUserConfig, JsonArray } from 'build-scripts';
...
const {
  constants: { MINIAPP, WECHAT_MINIPROGRAM, BYTEDANCE_MICROAPP, BAIDU_SMARTPROGRAM, KUAISHOU_MINIPROGRAM },
} = miniappBuilderShared;
...
interface IRaxAppUserConfig extends IUserConfig {
  targets: string[];
  store: string | number | boolean | Json | Date | JsonArray;
  web?: any;
  experiments: {
...
const getBuiltInPlugins: IGetBuiltInPlugins = (userConfig: IRaxAppUserConfig) => {
  ...
  const builtInPlugins: IPluginList = [['build-plugin-app-core', coreOptions], 'build-plugin-rax-app', 'build-plugin-ice-config'];
