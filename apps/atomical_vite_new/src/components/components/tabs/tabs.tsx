...
export const Tabs: FC<TabsProps> = (p) => {
  ...
  return withNativeProps(
    props,
    <div className="tabs">
      <div className={`tabs-header py-2 relative`}>
        <animated.div
        ...
        >
          <animated.div
            ref={activeLineRef}
            className={`tabs-tab-line h-[2px] bg-primary absolute bottom-0 left-0`}
            style={{
              width:
          ...
          {panes.map((pane) =>
            withNativeProps(
              pane.props,
              <div key={pane.key} className={"px-2"}>
                <div
                  onClick={() => {
                    const { key } = pane;
